...
- `render()` calcula `hashlib.sha256(mermaid_source.encode()).hexdigest()[:16]` y comprueba si `<output_dir>/<hash>.svg` ya existe. Si existe, lo retorna sin invocar nada (caché). Si no, invoca `npx --yes @mermaid-js/mermaid-cli -i <tmp.mmd> -o <output_dir>/<hash>.svg -b white` vía `subprocess.run`, y retorna el path resultante.
- En `__init__`, verifica disponibilidad de Node con `shutil.which("npx")`. Si no está disponible, lanza `RuntimeError` con un mensaje claro (ver sección "Verificación de Node").
...
- **`test_mermaid_renderer.py`** (nuevo): caché hit (segunda llamada con el mismo texto no invoca `subprocess`, mockeado), caché miss (invoca `subprocess.run` con los argumentos esperados), error claro cuando `shutil.which` retorna `None`. Sin depender de Node real — todo mockeado, para no romper CI/máquinas sin Node instalado.
