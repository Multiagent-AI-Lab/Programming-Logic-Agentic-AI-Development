...
# Regenerar los 12 notebooks tras editar cualquier lecciones/UNIDAD_*.md o EXTRA_*.md
...
- **`PseudocodeAgent`** — traduce entre pseudocódigo UCEMICH (la sintaxis propia del curso: `SI/SINO/FIN_SI`, `PARA/FIN_PARA`, `MIENTRAS/FIN_MIENTRAS`, `FUNCIÓN/RETORNAR/FIN_FUNCIÓN`, definida en `lecciones/UNIDAD_2_METODOLOGIA_ALGORITMOS_PRUEBAS.md`), Mermaid, y esqueletos Python.
...
- **`TutorAgent`** — el único agente con LLM: RAG semántico con ChromaDB sobre los `.md` de `lecciones/` (indexados por sección, citando fuente exacta) + Gemini para responder, con debugger socrático (`_diagnose_error`) y memoria episódica local (`_add_episode`/`_retrieve_relevant_episodes`, coincidencia por prefijo). Usa el SDK `google.genai` (no el deprecado `google.generativeai`); el `Client` se crea de forma perezosa dentro de `ask()` (nunca en `__init__` ni a nivel de módulo), leyendo `GEMINI_API_KEY` de `os.environ` en ese momento — así `TutorAgent` puede instanciarse sin key (solo `ask()` retorna un mensaje de error controlado con el contexto local igual), y no depende del orden en que se ejecuten celdas de notebook que configuren la key después del import. La colección de ChromaDB usa `SentenceTransformerEmbeddingFunction` con el modelo multilingüe `paraphrase-multilingual-MiniLM-L12-v2` (constante `EMBEDDING_MODEL_NAME`) en vez del default de ChromaDB (`all-MiniLM-L6-v2`, entrenado casi exclusivamente en inglés) — el default daba resultados de RAG muy pobres en español para el contenido del curso. `chroma_path` y `memory_path` son inyectables en el constructor específicamente para poder aislar tests con `tmp_path` — cualquier ruta de archivo/directorio nueva que se agregue a un agente debe seguir ese mismo patrón, no hardcodear rutas. `_get_markdown_files()` indexa tanto `UNIDAD_*.md` como `EXTRA_*.md`. Además del contenido de las lecciones, `_build_index()` indexa el abstract público (vía la API gratuita de Crossref, sin API key) de cada DOI citado en el texto de cualquier lección — un mismo DOI citado en varios archivos se consulta una sola vez y se indexa como un único documento (`_extract_dois`, `_fetch_abstract`), nunca como texto crudo mostrado al alumno, sino como contexto adicional para que Gemini fundamente mejor sus respuestas.
- **`ContentAuditorAgent`** — heurístico, sin LLM: audita el contenido pedagógico de un `UNIDAD_*.md` en 5 dimensiones (LaTeX malformado, ciclo Hilo de Oro incompleto, código de ejemplo sin docstring/type hints o con riesgos OWASP, alineación curricular contra el programa oficial en `docs/legado/`, e invariantes estructurales: consistencia de las celdas de auto-evaluación — unit_number correcto, %%writefile con definición previa — y reglas generales del Hilo de Oro: fences de código balanceados, celda de setup git-clone presente). Un test de gate en `tests/test_content_auditor_agent.py` corre esta 5ª dimensión sobre las 9 unidades reales del curso y falla si alguna tiene un hallazgo — las otras 4 dimensiones siguen siendo puramente informativas. Reutiliza `extract_fenced_blocks()` (función a nivel de módulo en `notebook_compiler_agent.py`) y `CodeAuditorAgent` internamente.

**Pipeline de contenido**: `lecciones/UNIDAD_*.md` y `lecciones/EXTRA_*.md` (fuente de verdad) → `convert_to_notebooks_smart.py` → `NotebookCompilerAgent.compile()` → `notebooks/*.ipynb`. Los notebooks nunca se editan a mano; cualquier cambio de contenido va al `.md` en `lecciones/` y se regenera. `convert_to_notebooks_smart.py` lista explícitamente los 12 archivos a convertir (`UNIDAD_0` a `UNIDAD_8`, más las 3 partes `EXTRA_PYTHON_IA_NANOTECNOLOGIA`).
...
- **Política de IA progresiva**: U1-U3 sin IA para código (solo consultas conceptuales), U4-U6 con Copiloto documentado, U7-U8 IA como herramienta principal con auditoría. Tabla completa en `lecciones/UNIDAD_0_ENTORNO_Y_PRIMER_PROGRAMA.md` sección 0.5.
